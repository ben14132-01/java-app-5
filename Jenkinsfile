pipeline {
    ...
    parameters {
        booleanParam(name: 'SNYK', defaultValue: false, description: 'Perform Snyk Scan')
    ...
    stages {
            ...
            steps {
                script {
                    bat """
                        ...
                        wsl git remote set-url origin https://%DOCKERHUB_USERNAME%:%GITHUB_TOKEN%@github.com/ben14132-01/java-app-5.git
            ...
            steps {
                script {
                    def response = bat(
                        ...
                        curl -X POST ^
                        -u admin:admin ^
                        "http://localhost:9001/api/projects/create?project=java-app-5&name=java-app-5"
                        ''',
                        returnStatus: true
                    )
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                dir('src/SpringAPI') {
                    bat '''
                    mvn clean verify sonar:sonar "-Dsonar.projectKey=java-app-5" "-Dsonar.projectName='java-app-5'" "-Dsonar.host.url=http://localhost:9001" "-Dsonar.token=%SONAR_TOKEN%"
                    '''
                }
            }
        }
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    def appName = 'java-app-5' //need to change appname
                    bat """
        ...
        //             bat """
        //                 wsl helm upgrade --install java-app ./charts/java-app-5 -n java --create-namespace --set image.tag=${commitId}
        //             """
        //         }
        //     }
        // }
 ...
 //add argocd password to jenkins credentials
        ...
        stage('ArgoCD connect repo') {
            steps {
                script {
                    bat """
                        wsl argocd repo list | wsl grep -q https://github.com/ben14132-01/java-app-5.git || wsl argocd repo add https://github.com/ben14132-01/java-app-5.git
                    """
                }
            }
        }
        ...
        stage('Deploy with ArgoCD') {
            steps {
                script {
                    ...
                    bat """
                        wsl argocd app get java-app-5 || wsl argocd app create java-app-5 \
                            --repo https://github.com/ben14132-01/java-app-5.git \
                            --path charts/java-app-5 \
                            --dest-namespace dev \
        ...
        stage('ArgoCD App Sync') {
            steps {
                script {
                    bat """
                        wsl argocd app sync java-app-5
                        wsl argocd app wait java-app-5 --timeout 180
                    """
                }
            }
        }
